...
public class CityGenerator : EditorWindow
{
    ...
    private void OnGUI()                            //에디터 창 UI 그리기
    {
        ...
        buildingSpacing = EditorGUILayout.FloatField("BuildingSpacing", buildingSpacing);  
    ...
    private void GenerateCity()           //도시 생성 함수
    {
        ...
        GameObject roadsParant = new GameObject("Roads");
        roadsParant.transform.SetParent(cityParent.transform, false);
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int z = 0; z < gridSizeZ; z++)
            {
                ...
                if (x % 2 ==0 || z % 2 ==0)
                {
                    CreateRoad(position, roadsParant.transform);
                }
                else
                {
