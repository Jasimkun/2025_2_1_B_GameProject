...
public class QuestManager : MonoBehaviour
{
    ...
    private QuestData currentQuest;                                 //진행 중인 퀘스트 데이터
    private int currentQuestIndex = 0;                              //퀘스트 목록 중에 진행 중인 번호
    ...
    void Start()
    {
        ...
        {
            StartQuest(availableQuests[0]);                 //시작 시 가지고 있는 첫 번째 배열의 퀘스트 진행
        }
    ...
    void Update()
    {
        if(currentQuest != null  && currentQuest.isActive)          //퀘스트 진행 중인지 체크 후
        {
            CheckDelivaryProgress();                                //퀘스트 진행 상태 함수 호출
    ...
    void CheckDelivaryProgress()
    {
        ...
        float distance = Vector3.Distance(player.position, currentQuest.delivaryPosition);  //유저와 도착지 거리를 계산한다.

        if (distance <= currentQuest.delivaryRedius)                        //유저의 거리가 도착 범위 안쪽인지 검사
        {
