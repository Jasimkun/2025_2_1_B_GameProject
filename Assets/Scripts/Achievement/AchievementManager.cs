...
public class AchievementManager : MonoBehaviour
{
    ...
    void Start()
    {
        ResetAllAchievements();                             //시작 시에 리셋 강제로 (테스트용) 나중에 배포 시에는 지운다
    ...
    public void UpdareAchievementUI()
    {
        if (achievementListContent == null || achievementSlotPrefab == null) return;        //두 내용 중 하나라도 없으면 모두 오류이기 때문에 return
    ...
    public void UpdateProgress(AchievementType type, int amount = 1)                //진행도 업데이트
    {
        ...
        foreach(AchievementData achievement in allAchievements)                     //해당 타이브이 모든 업적 체크
        {
