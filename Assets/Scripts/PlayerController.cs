...
public class PlayerController : MonoBehaviour
{
    ...
    void HandleMovement()               //이동 함수 제작
    {
        ...
        if ((isAttaciking && !canMoveWhileAttacking) || isLanding)
        {
            currentSpeed = 0;
            return;
        }
    ...
    void CheckGrounded()
    {
        ...
        if(!isGrounded && wasGrounded)           //지금 프레임은 땅이 아니고, 이전 프레임은 땅인 상태
        {
            Debug.Log("떨어지기 시작");
        }
    ...
    void HandleJump()
    {
        ...
        if(!isGrounded)      //땅 위에 있지 않을 경우 중력 적용
        {
    ...
    void HandleLanding()
    {
        if (isLanding)
        {
            landingTimer -= Time.deltaTime;  //랜딩 타이머 시간만큼 못 움직이게
    ...
    void HandleAttack()
    {
        ...
        if(Input.GetKeyDown(KeyCode.Alpha1) && !isAttaciking)   //공격 중이 아닐 때 키를 누르면 공격
        {
            isAttaciking = true;                                //공격중 표시
